...
public  Vector3       Center        = new Vector3(ProcGennerMultiplayer.MapSize_X/2, ProcGennerMultiplayer.MapSize_Y/2, ProcGennerMultiplayer.MapSize_Z/2);
    ...
    public void GenerateScene()
    {
        ...
        {            
            ...
            this.m_Grid = new BlockDef[ProcGennerMultiplayer.MapSize_X, ProcGennerMultiplayer.MapSize_Y, ProcGennerMultiplayer.MapSize_Z];
            this.m_Vertices = new List<Graphs.Vertex>();
            this.m_RoomVerts = new Dictionary<Graphs.Vertex, List<GameObject>>();
            
            // Initialize the grid
            for (int i=0; i<ProcGennerMultiplayer.MapSize_X; i++)
                for (int j=0; j<ProcGennerMultiplayer.MapSize_Y; j++)
                    for (int k=0; k<ProcGennerMultiplayer.MapSize_Z; k++)
        ...
        Bounds navmeshbounds = new Bounds(Vector3.zero, Center*2*ProcGennerMultiplayer.GridScale);
    ...
    Vector3Int GenerateRooms(LevelType ltype)
    {
        ...
        {
            bool overlap = false;
            int yval = ProcGennerMultiplayer.MapSize_Y/2;
            size = GenerateRoomVector();
            
            // Given a 1/4 chance, generate a room not at Y=0
            if (Random.Range(0, 4) == 0)
                yval = Random.Range(0, ProcGennerMultiplayer.MapSize_Y-1-ProcGennerMultiplayer.MaxRoomSize_Y);
            coord = new Vector3Int(
                (int)Random.Range(0, ProcGennerMultiplayer.MapSize_X-1-ProcGennerMultiplayer.MaxRoomSize_X), 
                (int)yval, 
                (int)Random.Range(0, ProcGennerMultiplayer.MapSize_X-1-ProcGennerMultiplayer.MaxRoomSize_Z)
            );
                ...
                for (int j=0; j<size.y && !overlap; j++)
                {
                    for (int k=0; k<size.x; k++)
                    {
                        ...
                        if ((this.m_Grid[gridx, gridy, gridz].type != BlockType.None) ||
                            (this.m_Grid[Mathf.Min(gridx+1, ProcGennerMultiplayer.MapSize_X-1), gridy, gridz].type != BlockType.None) ||
                            (this.m_Grid[Mathf.Max(gridx-1, 0), gridy, gridz].type != BlockType.None) ||
                            (this.m_Grid[gridx, Mathf.Min(gridy+1, ProcGennerMultiplayer.MapSize_Y-1), gridz].type != BlockType.None) ||
                            (this.m_Grid[gridx, Mathf.Max(gridy-1, 0), gridz].type != BlockType.None) ||
                            (this.m_Grid[gridx, gridy, Mathf.Min(gridz+1, ProcGennerMultiplayer.MapSize_Z-1)].type != BlockType.None) ||
    ...
    Vector3Int GenerateRoomVector()
    {
        return new Vector3Int(
            Random.Range(ProcGennerMultiplayer.MinRoomSize_X, ProcGennerMultiplayer.MaxRoomSize_X+1),
            Random.Range(ProcGennerMultiplayer.MinRoomSize_Y, ProcGennerMultiplayer.MaxRoomSize_Y+1),
            Random.Range(ProcGennerMultiplayer.MinRoomSize_Z, ProcGennerMultiplayer.MaxRoomSize_Z+1)
        );
    }
    ...
    bool CanPlaceCorridorWall(Vector3Int pos, Vector3Int dir)
    {
        ...
        if (check.x < 0 || check.x >= ProcGennerMultiplayer.MapSize_X || check.y < 0 || check.y >= ProcGennerMultiplayer.MapSize_Y || check.z < 0 || check.z >= ProcGennerMultiplayer.MapSize_Z)
            return true;
    ...
    {
        return Instantiate(this.m_CorridorStairPrefab, (pos-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorStairPrefab.transform.rotation*angle);
    }
    ...
    void GenerateCorridors()
    {
        ...
        {
            Vector3Int gridsize = new Vector3Int(ProcGennerMultiplayer.MapSize_X, ProcGennerMultiplayer.MapSize_Y, ProcGennerMultiplayer.MapSize_Z);
    ...
    void GenerateWalls()
    {
        ...
        for (int i=0; i<this.m_Corridors.Count; i++)
        {
            ...
            {
                instobj = Instantiate(this.m_CorridorPrefab_4Way, (cdef.position-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorPrefab_4Way.transform.rotation);
                cdef.prefab = instobj;
                continue;
            }
            ...
            {
                instobj = Instantiate(this.m_CorridorPrefab_T, (cdef.position-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorPrefab_T.transform.rotation);
                cdef.prefab = instobj;
                continue;
            }
            ...
            if (!CorridorConnectedLeft(cdef) && CorridorConnectedTop(cdef) && CorridorConnectedRight(cdef) && CorridorConnectedBottom(cdef))
            {
                instobj = Instantiate(this.m_CorridorPrefab_T, (cdef.position-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorPrefab_T.transform.rotation*Quaternion.Euler(0, 0, -90));
                cdef.prefab = instobj;
                continue;
            }
            ...
            if (CorridorConnectedLeft(cdef) && CorridorConnectedTop(cdef) && CorridorConnectedRight(cdef) && !CorridorConnectedBottom(cdef))
            {
                instobj = Instantiate(this.m_CorridorPrefab_T, (cdef.position-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorPrefab_T.transform.rotation*Quaternion.Euler(0, 0, 180));
                cdef.prefab = instobj;
                continue;
            }
            ...
            if (!CorridorConnectedLeft(cdef) && !CorridorConnectedTop(cdef) && CorridorConnectedRight(cdef) && CorridorConnectedBottom(cdef))
            {
                instobj = Instantiate(this.m_CorridorPrefab_Curve, (cdef.position-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorPrefab_Curve.transform.rotation*Quaternion.Euler(0, 0, -90));
                cdef.prefab = instobj;
                continue;
            }
            ...
            if (CorridorConnectedLeft(cdef) && !CorridorConnectedTop(cdef) && !CorridorConnectedRight(cdef) && CorridorConnectedBottom(cdef))
            {
                instobj = Instantiate(this.m_CorridorPrefab_Curve, (cdef.position-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorPrefab_Curve.transform.rotation*Quaternion.Euler(0, 0, 0));
                cdef.prefab = instobj;
                continue;
            }
            ...
            if (!CorridorConnectedLeft(cdef) && CorridorConnectedTop(cdef) && CorridorConnectedRight(cdef) && !CorridorConnectedBottom(cdef))
            {
                instobj = Instantiate(this.m_CorridorPrefab_Curve, (cdef.position-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorPrefab_Curve.transform.rotation*Quaternion.Euler(0, 0, 180));
                cdef.prefab = instobj;
                continue;
            }
            ...
            {
                instobj = Instantiate(this.m_CorridorPrefab_Straight, (cdef.position-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorPrefab_Straight.transform.rotation);
                cdef.prefab = instobj;
                continue;
            }
            ...
            {
                instobj = Instantiate(this.m_CorridorPrefab_Straight, (cdef.position-Center)*ProcGennerMultiplayer.GridScale, this.m_CorridorPrefab_Straight.transform.rotation*Quaternion.Euler(0, 0, -90));
                cdef.prefab = instobj;
                continue;
            }
...
void PlaceObjectsInRooms()
    {
        ...
        public virtual void OnDrawGizmos()
        {
            ...
            Gizmos.DrawWireCube(-(new Vector3(1, -1, 1)*ProcGennerMultiplayer.GridScale)/2, (new Vector3(ProcGennerMultiplayer.MapSize_X, ProcGennerMultiplayer.MapSize_Y, ProcGennerMultiplayer.MapSize_Z)*ProcGennerMultiplayer.GridScale));
            
            if (this.m_Vertices != null && this.m_Vertices.Count > 0)
            {
                Gizmos.color = Color.white;
                for (int i=0; i<this.m_Vertices.Count; i++)
                    Gizmos.DrawWireSphere(-(new Vector3(1, 0, 1)*ProcGennerMultiplayer.GridScale)/2 + (this.m_Vertices[i].Position-Center)*ProcGennerMultiplayer.GridScale, 3.0f);
            }
            ...
            if (this.m_SelectedEdges != null && this.m_Delaunay.Edges != null && this.m_Delaunay.Edges.Count > 0)
            {
                ...
                foreach (var edge in this.m_SelectedEdges)
                    Gizmos.DrawLine((edge.U.Position-Center)*ProcGennerMultiplayer.GridScale, (edge.V.Position-Center)*ProcGennerMultiplayer.GridScale);
            }
        }
    #endif
}